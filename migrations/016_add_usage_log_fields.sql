ALTER TABLE `logs`
  ADD COLUMN `username` varchar(128) DEFAULT '' AFTER `created_at`,
  ADD COLUMN `token_name` varchar(128) DEFAULT '' AFTER `username`,
  ...
  ADD KEY `idx_logs_user_created_model` (`user_id`, `created_at`, `model_name`);
