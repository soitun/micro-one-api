...
func newJobHarness(t *testing.T, repo *mockReconRepo, notifier Notifier, recipients []string) *ReconciliationJob {
	t.Helper()
...
func TestReconciliationJob_DiscrepancyTriggersNotify(t *testing.T) {
	...
	n := &recordingNotifier{}
	job := newJobHarness(t, repo, n, []string{"admin", "ops"})

	job.runReconciliation(context.Background())

	require.Len(t, n.calls, 2, "one notification per recipient")
	for _, call := range n.calls {
		assert.Equal(t, "event", call.notifyType)
...
func TestReconciliationJob_DefaultRecipientsWhenNoneConfigured(t *testing.T) {
	...
	n := &recordingNotifier{}
	// Pass no WithRecipients option to keep the constructor default of
	// ["admin"].
	...
	assert.Equal(t, "admin", n.calls[0].recipient)
}
